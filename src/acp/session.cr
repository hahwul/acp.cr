...
module ACP
  class Session
    ...
    # Available modes for this session (may be nil if agent doesn't support modes).
    getter modes : Array(Protocol::ModeOption)?
    ...
    # - `mcp_servers` — optional MCP server references.
    #
    ...
    def self.load(
      ...
      result = client.session_load(session_id, cwd)
      new(client, result.session_id, result.modes, result.config_options)
    end
    ...
    def initialize(
      @client : Client,
      @id : String,
      @modes : Array(Protocol::ModeOption)? = nil,
    ...
    def prompt(*texts : String) : Protocol::SessionPromptResult
      blocks = texts.map { |t| Protocol::TextContentBlock.new(t).as(Protocol::ContentBlock) }
      prompt(blocks.to_a)
    end
  ...
  class PromptBuilder
    ...
    # Adds an image content block from a URL.
    def image(url : String, mime_type : String? = nil) : self
      @blocks << Protocol::ImageContentBlock.new(url: url, mime_type: mime_type)
      self
    end
    ...
    # Adds an audio content block from a URL.
    def audio(url : String, mime_type : String? = nil) : self
      @blocks << Protocol::AudioContentBlock.new(url: url, mime_type: mime_type)
      self
    end
